...
const addModal = props => {
  ...
  useEffect(() => {
    let obj = {
      ...
      resumeFile: [props.scmDialog.params.resume]
    }
  ...
  const okHandle = () => {
    FormRef.validateFields().then(values => {
      let urlApi = null,data,
        info = '';
      values.initialGraduationTime =
        values.initialGraduationTime && values.initialGraduationTime.format('YYYY-MM-DD HH:mm:ss').toString();
        values.joinTime=values.joinTime&&values.joinTime.format('YYYY-MM-DD HH:mm:ss').toString();
      if (props.scmDialog.params.id) {
        (urlApi = innerEdit), (info = '编辑成功');
        values.id = props.scmDialog.params.id;
        data=checkValue(values)
      } else {
        (urlApi = innerAdd), (info = '新增成功');
        data=checkValue(values)
      }
    ...
    <Card>
      ...
      >
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8}>
            <Form.Item
              label="参加时间"
              name="joinTime"
              rules={[{ required: true, message: '请选择参加时间' }]}
            >
              <DatePicker />
            </Form.Item>
          </Col>
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8}>
            <Form.Item
              label="专业年限"
              name="majorYear"
              rules={[{ required: true, message: '请选择专业年限' }]}
            >
              <Input />
            </Form.Item>
          </Col>
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8}>
            <Form.Item
              ...
              rules={[{ required: true, message: '请选择推荐岗位' }]}
            >
              <Input />
            </Form.Item>
          </Col>
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8}>
            <Form.Item
              label="计算机熟练程度"
              name="computerSkill"
              rules={[{ required: true, message: '请选择计算机熟练程度' }]}
            >
              <FilterSelect allowClear="true">
                <Option key='熟练'>熟练</Option>
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8}>
            <Form.Item label="简历上传" name="file">
